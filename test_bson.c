#include "bson.h"
#include <assert.h>
#include <string.h>

void test_bson_decode(){
    err_t err;

    char data[] = {0x08, 0x00, 0x00, 0x00, 0x04, 0x04, 0x04, 0x00};
    bson_t* obj = bson_decode(data, 8, &err);

    assert(obj->data_len==8);
    for(int i=0;i<8;i++){
        assert(obj->data[i]==data[i]);
    }

}

void test_bson_get_val_string(){

    char data[] = {
        0x16, 0x00, 0x00, 0x00,                                // total document size
        0x02,                                                  // 0x02 = type String
        'h', 'e', 'l', 'l', 'o', 0x00,                         // field name
        0x06, 0x00, 0x00, 0x00, 'w', 'o', 'r', 'l','d', 0x00, // field value
        0x00 
    };

    err_t err;
    bson_t* obj = bson_decode(data, 22, &err);

    int len;
    char* str = bson_get_val_string(obj, "hello", NULL, &len);
    assert(len==6);
    assert(strcmp(str,"world")==0);

}

void test_bson_get_vals(){

    /*
        {"hello":"world", "william":5, "age":1.234}
    */
    err_t err;

    char data[] = {
      0x30, 0x00, 0x00, 0x00, 0x02, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x00, 0x06,
      0x00, 0x00, 0x00, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0x00, 0x10, 0x77, 0x69,
      0x6c, 0x6c, 0x69, 0x61, 0x6d, 0x00, 0x05, 0x00, 0x00, 0x00, 0x01, 0x61,
      0x67, 0x65, 0x00, 0x58, 0x39, 0xb4, 0xc8, 0x76, 0xbe, 0xf3, 0x3f, 0x00
    };

    bson_t* obj = bson_decode(data, 48, &err);

    int len;
    char* str = bson_get_val_string(obj, "hello", NULL, &len);
    assert(strcmp(str,"world")==0);

    int val = bson_get_val_int32(obj, "william", NULL);
    assert(val==5);

    double dval = bson_get_val_double(obj, "age", NULL);
    assert(dval==1.234);

}


void test_bson_to_json(){

    char* json_str;
    bson_t* obj;
    err_t err;

    /*
        {"hello":"world", "william":5, "age":1.234}
    */

    char data[] = {
      // document_size
      0x30, 0x00, 0x00, 0x00, 

      // "hello":"world"
      0x02, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x00, 0x06, 0x00, 0x00, 0x00, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0x00, 

      // "william":5
      0x10, 0x77, 0x69, 0x6c, 0x6c, 0x69, 0x61, 0x6d, 0x00, 0x05, 0x00, 0x00, 0x00, 

      // "age":1.234
      0x01, 0x61, 0x67, 0x65, 0x00, 0x58, 0x39, 0xb4, 0xc8, 0x76, 0xbe, 0xf3, 0x3f, 

      0x00
    };

    
    obj = bson_decode(data, 48, &err);
    json_str = bson_to_json(obj, &err);
    assert(strcmp(json_str,"{\"hello\":\"world\",\"william\":5,\"age\":1.234000}")==0);

    /*

        {"height":78, "customer":true, "name":{"first":"William", "last":"Schultz" } }
    */
    
    char data_embedded_doc[] = {
          0x4c, 0x00, 0x00, 0x00, 0x10, 0x68, 0x65, 0x69, 0x67, 0x68, 0x74, 0x00,
          0x4e, 0x00, 0x00, 0x00, 0x08, 0x63, 0x75, 0x73, 0x74, 0x6f, 0x6d, 0x65,
          0x72, 0x00, 0x01, 0x03, 0x6e, 0x61, 0x6d, 0x65, 0x00, 0x2a, 0x00, 0x00,
          0x00, 0x02, 0x66, 0x69, 0x72, 0x73, 0x74, 0x00, 0x08, 0x00, 0x00, 0x00,
          0x57, 0x69, 0x6c, 0x6c, 0x69, 0x61, 0x6d, 0x00, 0x02, 0x6c, 0x61, 0x73,
          0x74, 0x00, 0x08, 0x00, 0x00, 0x00, 0x53, 0x63, 0x68, 0x75, 0x6c, 0x74,
          0x7a, 0x00, 0x00, 0x00
    };

    unsigned int len = 76;

    obj = bson_decode(data_embedded_doc, 76, &err);
    json_str = bson_to_json(obj, &err);
    assert(strcmp(json_str, "{\"height\":78,\"customer\":true,\"name\":{\"first\":\"William\",\"last\":\"Schultz\"}}")==0);

}


int main(int argc, char const *argv[])
{

    /* code */
    test_bson_decode();
    test_bson_get_val_string();
    test_bson_get_vals();
    test_bson_to_json();

    return 0;
}

